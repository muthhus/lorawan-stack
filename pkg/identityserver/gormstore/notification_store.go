...
func (s *notificationStore) CreateNotification(ctx context.Context, pb *ttnpb.Notification, receiverIDs []*ttnpb.UserIdentifiers) (*ttnpb.Notification, error) {
	...
	model.Data, err = jsonpb.TTN().Marshal(pb.Data)
	if err != nil {
		return nil, err
	}
	...
	if pb.SenderIds != nil {
		...
		model.SenderID = senderModel.PrimaryKey()
	}
...
func (s *notificationStore) UpdateNotificationStatus(ctx context.Context, receiverIDs *ttnpb.UserIdentifiers, ids []string, status ttnpb.NotificationStatus) error {
	userIDQuery := s.query(ctx, Account{}).
		Select("account_id").
		Where("uid = ?", receiverIDs.GetUserId()).
		SubQuery()
